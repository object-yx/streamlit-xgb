...
if st.button("预测"):
    ...
    if predicted_class == 1:
        shap.force_plot(explainer_shap.expected_value, shap_values[1][0],pd.DataFrame([feature_names], columns = feature_names),matplotlib=True)
    else:
        shap.force_plot(explainer_shap.expected_value, shap_values[0][0],pd.DataFrame([feature_names], columns = feature_names),matplotlib=True)
